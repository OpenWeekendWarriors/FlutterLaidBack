...
class CircularCountDownTimerState extends State<CircularCountDownTimer> with TickerProviderStateMixin {
  ...
  String get time {
    if (widget.isReverse && _controller!.isDismissed) {
      if (widget.textFormat == CountdownTextFormat.MM_SS) {
        ...
        return "0";
      } else if (widget.textFormat == CountdownTextFormat.HH_MM) {
        return widget.initialDuration.secToHrMinString;
      } else {
        return "00:00:00";
      }
    } else {
  ...
  String _getTime(Duration duration) {
    ...
    if (widget.textFormat == CountdownTextFormat.HH_MM) {
                        ...
                        ? Align(
                            child: Column(
                              ...
                              children: [
                                ...
                                CustomText(
                                  'finish time ${DateTime.now().add(Duration(seconds: widget.initialDuration)).timeFormat1}',
                                  size: 12,
                                  color: Colors.white,
                                ),
                                const SizedBox()
